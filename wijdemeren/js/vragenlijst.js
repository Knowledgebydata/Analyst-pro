    ...
    var SECTIES = [
        {
            ...
            vragen: [
                ...
                { key: 'idVoorkant', label: 'Foto identiteitsbewijs — voorkant', type: 'foto' },
                { key: 'idAchterkant', label: 'Foto identiteitsbewijs — achterkant', type: 'foto' },
            ],
        },
        {
            key: 'woonsituatie',
        ...
        return new Promise(function (resolve) {
            ...
            input.addEventListener('change', async function () {
                ...
                try {
                    ...
                    var resized = await resizeImage(dataUrl, 1600);
                    resolve({
                        naam: file.name,
                        type: file.type,
                        data: resized,
    ...
    function openForm(locatieSlug, locatieNaam, pandLabel) {
        ...
        function renderSectie() {
            ...
            sectie.vragen.forEach(function (vraag) {
                ...
                } else if (vraag.type === 'foto') {
                    html += '  <div class="vl-foto-wrap" data-foto-key="' + vraag.key + '">';
                    if (huidig && huidig.data) {
                        html += '    <img class="vl-foto-preview" src="' + huidig.data + '" alt="' + escapeHtml(vraag.label) + '">';
                        html += '    <div class="vl-foto-actions">';
                        html += '      <button class="btn btn--sm" data-foto-retake="' + vraag.key + '">Opnieuw</button>';
                        html += '      <button class="btn btn--sm btn--danger" data-foto-remove="' + vraag.key + '">Verwijderen</button>';
                        html += '    </div>';
                    } else {
                        html += '    <button class="btn btn--sm btn--primary" data-foto-capture="' + vraag.key + '">Maak foto</button>';
                    }
                    html += '  </div>';
            ...
            // Foto capture knoppen
            modal.querySelectorAll('[data-foto-capture]').forEach(function (btn) {
                btn.addEventListener('click', async function () {
                    var key = btn.getAttribute('data-foto-capture');
                    var foto = await captureOnePhoto();
                    if (foto) {
                        antwoorden[key] = foto;
                        renderSectie(); // Herrender om preview te tonen
                    }
                });
            });

            // Foto opnieuw maken
            modal.querySelectorAll('[data-foto-retake]').forEach(function (btn) {
                btn.addEventListener('click', async function () {
                    var key = btn.getAttribute('data-foto-retake');
                    var foto = await captureOnePhoto();
                    if (foto) {
                        antwoorden[key] = foto;
                        renderSectie();
            ...
            // Foto verwijderen
            modal.querySelectorAll('[data-foto-remove]').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    var key = btn.getAttribute('data-foto-remove');
                    delete antwoorden[key];
                    renderSectie();
                });
            });
    ...
    function showDetail(vl) {
        ...
        SECTIES.forEach(function (sectie) {
            ...
            sectie.vragen.forEach(function (vraag) {
                ...
                // Foto-type: toon thumbnail
                if (vraag.type === 'foto' && val && val.data) {
                    html += '  <div class="vl-detail-rij">';
                    html += '    <span class="vl-detail-label">' + escapeHtml(vraag.label) + '</span>';
                    html += '    <img class="vl-foto-thumb" src="' + val.data + '" alt="' + escapeHtml(vraag.label) + '">';
                    html += '  </div>';
                    return;
                }
